try {
	;(() => {
		function a(e) {
			if (!e) return
			const t = e.getAttribute('tabindex') !== null,
...
`,
			...
			const s = () => !t || t.classList.remove('show'),
				d = () => {
					!t || Number.parseFloat(getComputedStyle(t).opacity) > 0 || (t.remove(), (t = void 0))
				}
			setTimeout(s, 1500),
				setTimeout(d, 2500),
		...
		function i(o) {
			o.querySelectorAll?.('.expressive-code .copy button').forEach((e) => e.addEventListener('click', u))
		}
		i(document)
