import { S as A } from './base-80a1f760.EHupSAwf.js'
import { g as $ } from './index.CSo-74ve.js'
import {
	_ as C,
	S as D,
	d as E,
	...
	e as x,
	a as z,
} from './SandpackSnippet.DQkj9R0A.js'
const B = {
...
var G = (() => {
		function r(s, a, n) {
			...
			;(this.type = s),
				...
				(this.protocol = n),
				(this._disposeMessageListener = this.protocol.channelListen((e) =>
						...
						return q(this, function (u) {
							switch (u.label) {
								...
								case 3:
									return (
										(t = u.sent()),
										(l = { type: this.getTypeId(), msgId: i.msgId, error: { message: t.message } }),
										this.protocol.dispatch(l),
										[3, 4]
									)
								case 4:
	...
	Y = (() => {
		function r(s, a) {
			;(this.globalListeners = {}),
				...
				(this.frameWindow = s.contentWindow),
				(this.origin = a),
				(this.globalListeners = []),
				(this.channelListeners = []),
		...
		return (
			...
			(r.prototype.dispatch = function (s) {
				this.frameWindow && this.frameWindow.postMessage(x({ $id: this.channelId, codesandbox: !0 }, s), this.origin)
			}),
			(r.prototype.globalListen = function (s) {
				if (typeof s != 'function') return () => {}
				var n = this.globalListenersCount
				return (
					(this.globalListeners[n] = s),
					this.globalListenersCount++,
					() => {
						delete this.globalListeners[n]
					}
				)
			}),
			(r.prototype.channelListen = function (s) {
				if (typeof s != 'function') return () => {}
				var n = this.channelListenersCount
				return (
					(this.channelListeners[n] = s),
					this.channelListenersCount++,
					() => {
						delete this.channelListeners[n]
					}
				)
			}),
			(r.prototype.eventListener = function (s) {
				if (s.source === this.frameWindow) {
					var a = s.data
					a.codesandbox &&
						(Object.values(this.globalListeners).forEach((n) => n(a)),
						a.$id === this.channelId && Object.values(this.channelListeners).forEach((n) => n(a)))
				}
			}),
			r
		)
...
function ae(r, s) {
	...
	var a = r.dependencies,
		...
		o = r.devDependencies,
		e = o === void 0 ? {} : o,
		i = h(h([], Object.keys(n), !0), Object.keys(e), !0),
		c = Object.keys(s),
		t = ['@adonisjs/framework', '@adonisjs/core']
	if (i.some((p) => t.indexOf(p) > -1)) return 'adonis'
	var l = ['nuxt', 'nuxt-edge', 'nuxt-ts', 'nuxt-ts-edge', 'nuxt3']
	if (i.some((p) => l.indexOf(p) > -1)) return 'nuxt'
	...
	if (i.some((p) => u.indexOf(p) > -1)) return 'apollo'
	...
	if (c.some((p) => p.endsWith('.re'))) return 'reason'
	var v = ['parcel-bundler', 'parcel']
	if (i.some((p) => v.indexOf(p) > -1)) return 'parcel'
	var d = ['@dojo/core', '@dojo/framework']
	if (i.some((p) => d.indexOf(p) > -1)) return '@dojo/cli-create-app'
	...
	if (i.some((p) => m.indexOf(p) > -1) || Object.keys(n).length >= ee) return 'node'
}
function ie(r) {
	var s = r.split('.')
...
var j,
	L = '-{{suffix}}',
	ne = 'https://'
		.concat((j = '2.19.8') === null || j === void 0 ? void 0 : j.replace(/\./g, '-'))
		.concat(L, '-sandpack.codesandbox.io/'),
	te = ((r) => {
		C(s, r)
		function s(a, n, o) {
			o === void 0 && (o = {})
			var e = r.call(this, a, n, o) || this
			if (
				((e.getTranspilerContext = () =>
					new Promise((c) => {
						var t = e.listen((l) => {
							l.type === 'transpiler-context' && (c(l.data), t())
						})
						e.dispatch({ type: 'get-transpiler-context' })
					})),
				(e.getTranspiledFiles = () =>
					new Promise((c) => {
						var t = e.listen((l) => {
							l.type === 'all-modules' && (c(l.data), t())
						})
						e.dispatch({ type: 'get-modules' })
					})),
				(e.bundlerURL = e.createBundlerURL()),
				...
				F(i, "The element '".concat(a, "' was not found")),
					(e.element = i),
					(e.iframe = document.createElement('iframe')),
					e.initializeElement()
			} else (e.element = a), (e.iframe = a)
			return (
				...
				(e.iframeProtocol = new Y(e.iframe, e.bundlerURL)),
				(e.unsubscribeGlobalListener = e.iframeProtocol.globalListen((c) => {
					c.type !== 'initialized' ||
						!e.iframe.contentWindow ||
						(e.iframeProtocol.register(),
						e.options.fileResolver &&
							(e.fileResolverProtocol = new G(
								'fs',
								(t) =>
									z(e, void 0, void 0, function () {
										return q(this, function (l) {
											if (t.method === 'isFile') return [2, this.options.fileResolver.isFile(t.params[0])]
											if (t.method === 'readFile') return [2, this.options.fileResolver.readFile(t.params[0])]
				...
				(e.unsubscribeChannelListener = e.iframeProtocol.channelListen((c) => {
					switch (c.type) {
						case 'start': {
							e.errors = []
							break
						...
						case 'action': {
							c.action === 'show-error' && (e.errors = h(h([], e.errors, !0), [N(c)], !1))
							break
						}
						case 'done': {
							e.status = 'done'
						...
						case 'state': {
							e.bundlerState = c.state
							break
						}
					}
				})),
				o.experimental_enableServiceWorker && e.serviceWorkerHandshake(),
				e
			)
		}
		return (
			(s.prototype.createBundlerURL = function () {
				var a,
					n = this.options.bundlerURL || ne
				if (this.options.bundlerURL) return n
				if (
					(this.options.teamId &&
						(n = n.replace('https://', 'https://' + this.options.teamId + '-') + '?cache='.concat(Date.now())),
					this.options.experimental_enableServiceWorker)
				) {
					var o = []
					o.push(Math.random().toString(36).slice(4)),
						(n = n.replace(
							L,
							'-'.concat(
								(a = this.options.experimental_stableServiceWorkerId) !== null && a !== void 0 ? a : o.join('-'),
							),
						))
				} else n = n.replace(L, '')
				return n
			}),
			(s.prototype.serviceWorkerHandshake = function () {
				var n = new MessageChannel(),
					o = this.iframe.contentWindow
				if (!o) throw new Error('Could not get iframe contentWindow')
				var e = n.port1
				e.onmessage = (c) => {
					if (typeof c.data == 'object' && c.data.$channel === f)
						switch (c.data.$type) {
							...
							case 'preview/request':
								this.handleWorkerRequest(c.data, e)
								break
						}
				}
				var i = () => {
					var c = { $channel: f, $type: 'preview/init' }
					o.postMessage(c, '*', [n.port2]), this.iframe.removeEventListener('load', i)
				}
				this.iframe.addEventListener('load', i)
			}),
			(s.prototype.handleWorkerRequest = function (a, n) {
				return z(this, void 0, void 0, function () {
					var o, e, i, c, t, l, u, v, d, m, p
					return q(this, function (b) {
						switch (b.label) {
							...
							case 1:
								return (
									b.trys.push([1, 4, , 5]),
									(e = new URL(a.url, this.bundlerURL).pathname),
									(i = {}),
									(c = this.getFiles()),
									(t = c[e]),
									t ? [3, 3] : [4, this.getTranspiledFiles()]
								)
							case 2:
								if (((l = b.sent()), (t = l.find((g) => g.path.endsWith(e))), !t)) return o(), [2]
								b.label = 3
							case 3:
								return (
									(u = t.code),
									i['Content-Type'] || ((v = ie(e)), (d = Z.get(v)), d && (i['Content-Type'] = d)),
									(m = { $channel: f, $type: 'preview/response', id: a.id, headers: i, status: 200, body: u }),
									n.postMessage(m),
									[3, 5]
								)
							case 4:
								return (p = b.sent()), console.error(p), o(), [3, 5]
							case 5:
								return [2]
						}
					})
			...
			(s.prototype.setLocationURLIntoIFrame = function () {
				var a,
					n = this.options.startRoute ? new URL(this.options.startRoute, this.bundlerURL).toString() : this.bundlerURL
				;(a = this.iframe.contentWindow) === null || a === void 0 || a.location.replace(n), (this.iframe.src = n)
			}),
			(s.prototype.destroy = function () {
				this.unsubscribeChannelListener(), this.unsubscribeGlobalListener(), this.iframeProtocol.cleanup()
			}),
			(s.prototype.updateOptions = function (a) {
				E(this.options, a) || ((this.options = a), this.updateSandbox())
			}),
			(s.prototype.updateSandbox = function (a, n) {
				var o, e, i, c
				a === void 0 && (a = this.sandboxSetup), (this.sandboxSetup = x(x({}, this.sandboxSetup), a))
				var t = this.getFiles(),
					l = Object.keys(t).reduce((d, m) => {
						var p
						return x(x({}, d), ((p = {}), (p[m] = { code: t[m].code, path: m }), p))
					}, {}),
					u = JSON.parse(M(this.sandboxSetup.dependencies, this.sandboxSetup.devDependencies, this.sandboxSetup.entry))
				try {
					u = JSON.parse(t['/package.json'].code)
				} catch (d) {
					console.error(W('could not parse package.json file: ' + d.message))
				}
				var v = Object.keys(t).reduce((d, m) => {
					var p
					return x(x({}, d), ((p = {}), (p[m] = { content: t[m].code, path: m }), p))
				}, {})
				this.dispatch(
					x(x({}, this.options), {
						type: 'compile',
						...
						template: this.sandboxSetup.template || ae(u, v),
						...
						logLevel: (c = this.options.logLevel) !== null && c !== void 0 ? c : D.Info,
			...
			(s.prototype.dispatch = function (a) {
				a.type === 'refresh' &&
					(this.setLocationURLIntoIFrame(),
					this.options.experimental_enableServiceWorker && this.serviceWorkerHandshake()),
					this.iframeProtocol.dispatch(a)
			}),
			(s.prototype.listen = function (a) {
				return this.iframeProtocol.channelListen(a)
			}),
			(s.prototype.getCodeSandboxURL = function () {
				var a = this.getFiles(),
					n = Object.keys(a).reduce((o, e) => {
						var i
						return x(x({}, o), ((i = {}), (i[e.replace('/', '')] = { content: a[e].code, isBinary: !1 }), i))
					}, {})
			...
			(s.prototype.getFiles = function () {
				var a = this.sandboxSetup
				return a.files['/package.json'] === void 0
					? P(a.files, a.dependencies, a.devDependencies, a.entry)
					: this.sandboxSetup.files
			}),
			(s.prototype.initializeElement = function () {
